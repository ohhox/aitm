  ...
  useEffect(() => {
    Promise.all([
      ...
      setForm({
        ...
        acceptanceCriteria: Array.isArray(criteria) ? criteria.join('\n') : String(criteria),
        notes: task.notes ?? '',
      })
      setRootTasks(project.tasks ?? [])
      setLoading(false)
    })
  }, [id, taskId])
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="h-[52px] bg-white border-b border-[#E2E6EC] flex items-center gap-2 px-5 shrink-0">
        ...
        <span className="text-sm font-semibold">Edit Task</span>
      </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white border border-[#E2E6EC] rounded-md overflow-hidden shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                <label className={labelClass}>Notes <span className="text-[#9CA3AF] font-normal normal-case">(hints สำหรับ AI)</span></label>
                <textarea
                  className={`${inputClass} resize-none`}
                  rows={2}
                  value={form.notes}
